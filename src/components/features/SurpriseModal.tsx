...
interface StepOption { emoji: string; label: string; value: string; isAny?: boolean; }
interface StepConfig  { key: StepKey; title: string; options: StepOption[]; }

const STEPS: StepConfig[] = [
  { key: 'temperature', title: 'تحب المشروب؟', options: [
    { emoji: '🔥', label: 'ساخن',   value: 'hot'  },
    { emoji: '🧊', label: 'بارد',   value: 'cold' },
    { emoji: '🎲', label: 'فاجئني', value: 'any', isAny: true },
  ]},
  { key: 'mood', title: 'إيه مزاجك؟', options: [
  ...
  { key: 'flavorFamily', title: 'تفضل إيه؟', options: [
  ...
  { key: 'sweetness', title: 'درجة الحلاوة؟', options: [
    ...
    <div className="flex gap-1.5 mb-4">
      {STEPS.map((_, i) => (
        <div key={i} className="flex-1 h-[3px] rounded-full transition-all duration-500"
          style={{ background: i <= current ? 'linear-gradient(90deg,#c9993d,#f0c862)' : 'var(--border-1)' }}
        />
      ))}
    </div>
  );
...
function OptionBtn({ opt, onSelect }: { opt: StepOption; onSelect: () => void }) {
  return (
    <motion.button
      whileTap={{ scale: 0.92 }} whileHover={{ scale: 1.03 }}
      onClick={onSelect}
      className="flex flex-col items-center justify-center gap-1.5 rounded-2xl py-3.5 px-2 w-full transition-colors"
      style={{
        background: opt.isAny ? 'var(--surface-1)' : 'linear-gradient(135deg,rgba(201,153,61,0.11),rgba(201,153,61,0.04))',
        border: `1px solid ${opt.isAny ? 'var(--border-1)' : 'rgba(201,153,61,0.22)'}`,
        minHeight: '68px',
    ...
    >
      ...
      <span className="text-xs font-semibold text-center leading-tight"
        style={{ color: opt.isAny ? 'var(--text-3)' : 'var(--text-1)' }}>
        {opt.label}
      </span>
    </motion.button>
  );
...
function QuestionsPhase({ currentStep, onSelect, onBack }: {
  currentStep: number; onSelect: (v: string) => void; onBack: () => void;
}) {
  const step = STEPS[currentStep];
  const regularOpts = step.options.filter(o => !o.isAny);
  const anyOpt      = step.options.find(o => o.isAny);
  const cols = regularOpts.length <= 2 ? 'grid-cols-2' : regularOpts.length === 3 ? 'grid-cols-3' : 'grid-cols-2';

  return (
    <div className="px-5 pb-6">
      <div className="flex items-center gap-2 mb-1" dir="rtl">
        <button onClick={onBack}
          className="flex items-center justify-center w-8 h-8 rounded-xl flex-shrink-0 transition-colors hover:opacity-80"
          aria-label="السابق"
          style={{ visibility: currentStep > 0 ? 'visible' : 'hidden', background: 'var(--surface-2)' }}
        >
          <ChevronRight className="w-4 h-4" style={{ color: 'var(--text-2)' }} />
        </button>
        <div className="flex-1"><ProgressBar current={currentStep} /></div>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <p className="text-[11px] text-center mb-1.5"
            style={{ color: 'var(--text-5)', fontFamily: "'Cairo',sans-serif" }}>
            السؤال {AR_NUM[currentStep]} من {AR_NUM[STEPS.length - 1]}
          </p>
          <h2 className="font-bold text-[22px] text-center mb-5" dir="rtl"
          ...
          {anyOpt && (
            <motion.button whileTap={{ scale: 0.97 }} onClick={() => onSelect(anyOpt.value)}
              className="w-full py-3 rounded-2xl flex items-center justify-center gap-2 transition-colors"
              style={{ background: 'var(--surface-1)', border: '1px solid var(--border-1)', fontFamily: "'Cairo',sans-serif" }}
            >
              <span className="text-lg select-none">{anyOpt.emoji}</span>
              <span className="text-sm font-medium" style={{ color: 'var(--text-3)' }}>{anyOpt.label}</span>
            </motion.button>
          )}
        </motion.div>
      </AnimatePresence>
    ...
    <div className="flex flex-col items-center justify-center py-12 px-6">
      <div className="relative mb-8">
        <motion.div animate={{ rotate: 360 }} transition={{ duration: 2.5, repeat: Infinity, ease: 'linear' }}
          ...
          style={{ background: 'conic-gradient(from 0deg, transparent 0%, #c9993d 40%, #f0c862 60%, transparent 100%)' }}
        />
      ...
      <motion.p animate={{ opacity: [0.55, 1, 0.55] }} transition={{ duration: 1.8, repeat: Infinity }}
        className="font-bold text-[15px] text-center mb-4" dir="rtl"
        style={{ color: 'var(--text-1)', fontFamily: "'Cairo',sans-serif" }}>
      ...
      <div className="flex gap-2">
        {[0, 1, 2].map(i => (
          <motion.div key={i} animate={{ y: [-5, 0, -5] }}
...
/* ── ResultPhase ───────────────────────────────────────── */
function ResultPhase({ result, onAnother }: { result: RecommendationResult; onAnother: () => void }) {
    ...
    <div>
      ...
      <div className="relative h-52 overflow-hidden">
        ...
        <div className="absolute inset-0"
          style={{ background: 'linear-gradient(to top,var(--bg-card) 5%,rgba(var(--bg-card-rgb),0.35) 55%,transparent 100%)' }} />

        <div className="absolute top-3 right-3">
          <span className="text-xs px-2.5 py-1 rounded-full"
            style={{ background: 'rgba(0,0,0,0.65)', color: '#f0c862', backdropFilter: 'blur(8px)', fontFamily: "'Cairo',sans-serif" }}>
            {catIcon} {catName}
          </span>
        </div>
        <div className="absolute top-3 left-3">
          <span className="text-[10px] px-2.5 py-1 rounded-full font-bold"
            style={{ background: 'rgba(201,153,61,0.2)', border: '1px solid rgba(201,153,61,0.4)', color: '#f0c862', backdropFilter: 'blur(8px)', fontFamily: "'Cairo',sans-serif" }}>
            ترشيح مثالي ✦
          </span>
        </div>
      ...
      <div className="px-5 pt-4 pb-6" dir="rtl">
        ...
        <div className="flex items-start gap-2.5 p-3.5 rounded-2xl mb-4"
          style={{ background: 'rgba(201,153,61,0.08)', border: '1px solid rgba(201,153,61,0.18)' }}>
          <Sparkles className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: '#c9993d' }} />
          <p className="text-xs leading-relaxed" style={{ color: '#c9993d', fontFamily: "'Cairo',sans-serif" }}>
            <span className="font-bold">سبب الترشيح: </span>{result.reason}
          </p>
        </div>

        {/* Add to cart */}
        <motion.button whileTap={{ scale: 0.97 }} onClick={() => setShowCustomizer(true)}
          className="w-full py-4 rounded-2xl font-bold text-sm mb-2.5 flex items-center justify-center gap-2"
          style={{ background: 'linear-gradient(135deg,#c9993d,#f0c862)', color: '#07070f', fontFamily: "'Cairo',sans-serif", boxShadow: '0 6px 24px rgba(201,153,61,0.32)', border: 'none', cursor: 'pointer' }}>
          <ShoppingBag className="w-4 h-4" />
          أضف للسلة
        </motion.button>

        <a href={waUrl} target="_blank" rel="noopener noreferrer"
          className="block w-full py-3.5 rounded-2xl text-center font-semibold text-sm mb-2.5"
          style={{ background: 'rgba(37,211,102,0.08)', border: '1px solid rgba(37,211,102,0.22)', color: '#25D366', fontFamily: "'Cairo',sans-serif", textDecoration: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
          📱 اطلب الآن عبر واتساب
        </a>

        <motion.button whileTap={{ scale: 0.97 }} onClick={onAnother}
          className="w-full py-3.5 rounded-2xl font-semibold text-sm flex items-center justify-center gap-2"
          style={{ background: 'var(--surface-1)', border: '1px solid var(--border-1)', color: 'var(--text-2)', fontFamily: "'Cairo',sans-serif" }}>
          <span>🔄</span>
          رشحلي مشروب تاني
        </motion.button>
      </div>
      ...
      <AnimatePresence>
        {showCustomizer && (
          <ItemCustomizerModal key={`surprise-customizer-${result.item.id}`}
            item={result.item} onClose={() => setShowCustomizer(false)} />
        )}
      </AnimatePresence>
    </div>
  );
  ...
  return (
    ...
    >
      <motion.div
        ...
        style={{ background: 'var(--bg-card)', border: '1px solid rgba(201,153,61,0.18)', boxShadow: 'var(--modal-shadow)', maxHeight: '92svh' }}
      >
        <div className="overflow-y-auto scrollbar-hide" style={{ maxHeight: '92svh' }}>
          ...
          <div className="flex justify-center pt-3 sm:hidden">
            <div className="w-10 h-1 rounded-full" style={{ background: 'var(--border-3)' }} />
          </div>
          ...
          {phase !== 'result' && (
            <div className="pt-5 pb-3 px-5 text-center">
              ...
              <p className="text-xs"
                style={{ color: 'var(--text-2)', fontFamily: "'Cairo',sans-serif" }}>
                {phase === 'loading' ? 'بدور على المشروب المثالي ليك...' : 'جاوب على ٤ أسئلة وهرشحلك أحسن حاجة'}
              </p>
            </div>
          )}
          ...
          <AnimatePresence mode="wait">
            ...
            {phase === 'result' && result && (
              <motion.div key="r" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.35 }}>
                <ResultPhase result={result} onAnother={() => startLoading(answers as UserAnswers, shownIds)} />
              </motion.div>
            )}
          </AnimatePresence>
        </div>
        ...
        <button onClick={onClose}
          className="absolute top-5 left-4 z-30 w-8 h-8 flex items-center justify-center rounded-full transition-colors hover:opacity-80"
